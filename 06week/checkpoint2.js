...
// Given 1000 digits of PI as strings, return an array of the digits as numbers
 const nums = strNums.map(numbers => Number(numbers));
// builds new array that takes the numbers in the strNums array and 
// rolls out the the function over each array 
// function = string data type into number datatype


const sumEvens = numbers.filter(number => {
  return (number % 2 == 0);
// filter goes through all of the numbers that find the 
/// values that are even numbers.
}).reduce((result, newItem) => {
...
// takes weather obj, pulls weather_state_name values and puts them into
// their own array
const weatherStates = weather.map((arr)=>{
    return arr['weather_state_name'];
  })
  console.log(weatherStates)
// searches through the 'min_temps' of each object 
// and searches for the one equal to 15.915
// returns it
  const idealTemp = weather.find((trueTemp)=>{
    if (trueTemp['min-temp'] === 15.915) {
      return trueTemp.id;
    }
  })
